...
export async function GET(req: NextRequest) {
  ...
  const results: Array<{ storeId: string; storeName: string; catalog: string; judgeme: string; skipped?: boolean }> = [];
  ...
  for (const store of stores) {
    ...
    if (running) {
      results.push({ storeId: store.id, storeName: store.name, catalog: "skipped", judgeme: "skipped", skipped: true });
      continue;
    }
    try {
      const [catalog, judgeme] = await Promise.all([syncCatalog(store.id, "scheduled"), syncJudgeme(store.id, "scheduled")]);
      results.push({ storeId: store.id, storeName: store.name, catalog: catalog.status, judgeme: judgeme.status });
    } catch (err) {
      console.error("[cron/sync] erreur inattendue", { storeId: store.id, error: err instanceof Error ? err.message : String(err) });
      results.push({ storeId: store.id, storeName: store.name, catalog: "error", judgeme: "error" });
    }
  }
